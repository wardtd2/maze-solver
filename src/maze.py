...
class Maze:
    ...
    def _solve_r(self, i, j):
        ...
        if self._cells[i][j].has_left_wall == False and i - 1 >= 0 and self._cells[i-1][j].visited == False:
            possible_directions.append((i - 1, j))
        # Right
        if self._cells[i][j].has_right_wall == False and i + 1 < self.num_cols and self._cells[i + 1][j].visited == False:
            possible_directions.append((i + 1, j))
        # Up
        if self._cells[i][j].has_top_wall == False and j - 1 >= 0 and self._cells[i][j - 1].visited == False:
            possible_directions.append((i, j - 1))
        # Down
        if self._cells[i][j].has_bottom_wall == False and j + 1 < self.num_rows and self._cells[i][j + 1].visited == False:
            possible_directions.append((i, j + 1))
        
        for direction in possible_directions:
            next_cell_x = direction[0]
            next_cell_y = direction[1]
            self._cells[i][j].draw_move(self._cells[next_cell_x][next_cell_y])
            if self._solve_r(next_cell_x, next_cell_y):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[next_cell_x][next_cell_y], True)
